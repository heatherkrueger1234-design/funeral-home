 ...
 *
 * This exists so that "download everything" genuinely means everything. The
 * JSON export lists photographs as URLs that only resolve while signed in,
 * which makes the export useless in exactly the situation someone takes one:
 * the account is going away, or the site is, and what they wanted was the
 ...
 *
 * Deliberately not implemented: ZIP64. The per-account storage ceiling is 2 GB,
 * well inside the 4 GB the 32-bit offset and size fields allow. `addFile`
 * throws rather than silently writing a corrupt archive if that ever changes.
 */
