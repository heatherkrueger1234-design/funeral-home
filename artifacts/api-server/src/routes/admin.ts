...
/**
 * Who is acting, for the audit. Carries the id as well as the address: the id
 * is the foreign key, the address is what keeps an audit line legible a year
 * later without a join.
 */
type PlatformActor = { id: number; email: string };

/**
 * The gate is Component 1's, and this file no longer has one of its own.
 *
 * What was here until Component 1 landed was an allowlist checked on top of a
 * *staff* session, which is why this router used to be mounted below
 * `requireAuth`. The real gate is a different account entirely — its own
 * table, its own cookie, its own resolver — so the mount moved above
 * `requireAuth` in `routes/index.ts` at the same time. A platform admin has
 * no staff session and would otherwise have been refused before ever
 * reaching this file.
 *
 * `requirePlatformAdmin` deliberately attaches neither `req.user` nor
 * `req.home`, so nothing below can call `tenant(req)` and quietly acquire a
 * scope that does not exist here.
 */
router.use("/admin", requirePlatformAdmin);

/** Narrowing helper, the same shape as `currentUser` and for the same reason. */
function actor(req: Parameters<typeof platformAdmin>[0]): PlatformActor {
  const admin = platformAdmin(req);
